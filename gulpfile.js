...
var resourceFilesPath = {
    ...
    style:SRC+"assets/less/app/*.less",
    less:SRC+"assets/less/**/*.less",
    lessToCss:SRC+"assets/less/**/*.less",
...
gulp.task(task.style,function(){
    gutil.log("[logs]"+task.style+"正在运行...");
    return gulp.src(resourceFilesPath.style, { base: 'src/main/resources/assets/less/app' })
        .pipe(less())
        .on('error', function(error)  {
            console.dir(error);
            this.emit('end');
...
gulp.task(task.lib,function(){
    ...
    libs = Object.keys(libs).map(function(module, i) {
        return gulp.src(['./node_modules/' + module + '/**/*'])
            .pipe(gulp.dest(destFilesPath.libs+'/'+module+'/'));
    });
});
...
gulp.task(task.watch,function(){
    gulp.watch(resourceFilesPath.less, [task.style]);
...
//默认的task
gulp.task("default",[task.clean,task.style,task.js,task.images,task.lib,task.plugins,task.jade,task.api]);
